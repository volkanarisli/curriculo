...
const WorkForm = ({ isResumeBuilder }) => {
    ...
    const addNewExperience = () => {
        if (!experiences) {
            ...
            setKeywords(prevState => ({ ...prevState, [experiences.length]: [] }))
        }

    }
    const removeExperience = (index) => {
        const newArr = experiences.filter((_, indexOfExperience) => indexOfExperience !== index)
        setExperiences([...newArr])
    }
    ...
    useEffect(() => {
        ...
        setExperiences([{ id: 0 }])
        setKeywords({ 0: [] })
    }, [isResumeBuilder, setExperiences])
