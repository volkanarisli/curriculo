                ...
                tabs.map((tab, key) => (
                    <div className="flex items-center" key={key}>
                        <span className={`mx-5 cursor-pointer py-1 px-3 rounded flex ${!tab.isSuccess && currentTab === key && 'hover:bg-gray-300'}
                         ${!tab.isSuccess && !(currentTab === key) && 'cursor-not-allowed'} ${tab.isSuccess && 'text-indigo-700'}`}
                            onClick={() => setTabFromNavigation(key)}
                        >
                            {
                                tab.isSuccess &&
                                <svg xmlns="http://www.w3.org/2000/svg" className="text-indigo-700 h-6 w-6 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
